...
create policy "Users read own profile" on public."User"
  for select using (auth.uid() = id or (select role from public."User" where id = auth.uid()) in ('teacher', 'admin'));
...
create policy "Certificates read own" on public."Certificate"
  for select using (auth.uid() = "studentId" or (select role from public."User" where id = auth.uid()) in ('teacher', 'admin'));
...
create policy "Submissions teacher update" on public."AssignmentSubmission"
  for update using ((select role from public."User" where id = auth.uid()) in ('teacher', 'admin'));
